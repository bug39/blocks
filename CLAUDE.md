...
3) curl -X POST http://localhost:8000/analyze -H "Content-Type: application/json" -d '{"pr_url":"<PR_URL>"}'
4) curl -X POST http://localhost:8000/analyze -H "Content-Type: application/json" -d '{"mode":"scan"}'
5) curl -X POST http://localhost:8000/wizard -H "Content-Type: application/json" -d '{"input":"If PR has no reviewers after 2 hours, request reviewers from CODEOWNERS"}'
